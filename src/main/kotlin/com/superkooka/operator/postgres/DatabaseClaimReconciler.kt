...
class DatabaseClaimReconciler(private val client: KubernetesClient) : Reconciler<DatabaseClaim> {

    override fun reconcile(resource: DatabaseClaim, context: Context<DatabaseClaim>): UpdateControl<DatabaseClaim> {
        val name = resource.metadata.name
        val namespace = resource.metadata.namespace
        val spec = resource.spec ?: run {
        ...
        val port = service.spec.ports.first().port
        ...
        logger.info { "DatabaseClaim '$databaseName' reconciled successfully" }

        return UpdateControl.updateStatus(resource)
    }

    fun createDatabase(host: String, port: Int, adminUser: String, adminPassword: String, dbName: String) {
